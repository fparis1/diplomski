...
colnames(airport_data) <- c("id", "name", "city", "country", "iata_code", 
                            ...
                            "type", "source")
airport_data <- airport_data[, c("icao_code", "latitude", "longitude")]
  ...
  tabPanel("Podaci",
           sidebarLayout(
             sidebarPanel(
               ...
               # Departure airport selection (needed for API)
               selectInput("airport_start", "Odaberite zračnu luku polaska:", 
                           choices = setNames(
                             names(airports_full_names), 
                             sapply(names(airports_full_names), function(icao) {
                               full_name <- airports_full_names[[icao]]
                               city_country <- sub(".*\\((.*)\\).*", "\\1", full_name)
                               paste(icao, "-", city_country)
                             })
                           )
               ),
               # Date and time inputs for flight
               dateInput("flight_date", "Datum leta:", value = Sys.Date()),
  ...
  observeEvent(input$fetch, {
    ...
    tryCatch({
      ...
      flights <- get_flight_data(
        departure_airport = input$airport_start,
